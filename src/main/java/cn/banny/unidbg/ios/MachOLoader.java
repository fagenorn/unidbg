...
public class MachOLoader extends AbstractLoader implements Memory, Loader, cn.banny.unidbg.ios.MachO {
    ...
    private void initializeTSD() {
        ...
        final UnicornPointer tsd = (UnicornPointer) thread.share(emulator.getPointerSize() == 8 ? 0xe0 : 0xa4); // tsd
                                                                                                                // size
        assert tsd != null;
        ...
        log.debug("initializeTSD tsd=" + tsd + ", thread=" + thread + ", environ=" + environ + ", vars=" + vars
                + ", locale=" + locale + ", gap=" + gap + ", errno=" + errno);
    }

    @Override
    protected Module loadInternal(LibraryFile libraryFile, WriteHook unpackHook, boolean forceCallInit)
            throws IOException {
    ...
    private MachOModule loadInternal(LibraryFile libraryFile, boolean forceCallInit, boolean checkBootstrap)
        ...
        for (MachOModule export : modules.values()) {
            if (!export.lazyLoadNeededList.isEmpty()) {
                log.info("Export module resolve needed library failed: " + export.name + ", neededList="
                        + export.lazyLoadNeededList);
            }
        }
    ...
    private MachOModule loadInternalPhase(LibraryFile libraryFile, ByteBuffer buffer, boolean loadNeeded,
            boolean checkBootstrap) throws IOException {
        ...
        case FAT_BE:
            ...
            for (MachO.FatArch arch : machO.fatHeader().fatArchs()) {
                if ((arch.cputype() == MachO.CpuType.ARM && emulator.getPointerSize() == 4)
                        || (arch.cputype() == MachO.CpuType.ARM64 && emulator.getPointerSize() == 8)) {
        ...
        if (checkBootstrap && !isExecutable && executableModule == null) {
            ...
            Module bootstrap = loadInternal(new URLibraryFile(url, "unidbg_bootstrap", DarwinResolver.LIB_VERSION),
                    false, false);
            // emulator.traceCode();
            ...
            case SEGMENT: {
                ...
                if (segmentCommand.filesize() > segmentCommand.vmsize()) {
                    throw new IllegalStateException(String.format(
                            "malformed mach-o image: segment load command %s filesize is larger than vmsize",
                            command.type()));
                }
                if (finalSegment) {
                ...
                if (segmentCommand.vmsize() < segmentCommand.filesize()) {
                    throw new IllegalStateException(
                            String.format("malformed mach-o image: segment %s has vmsize < filesize", command.type()));
                }
        ...
        if (log.isDebugEnabled()) {
            log.debug("start map dyid=" + dyId + ", base=0x" + Long.toHexString(loadBase) + ", size=0x"
        ...
        for (MachO.LoadCommand command : machO.loadCommands()) {
            ...
            case SEGMENT: {
                ...
                if ("__PAGEZERO".equals(segmentCommand.segname())) {
                    regions.add(new MemRegion(begin, begin + segmentCommand.vmsize(), 0, libraryFile,
                            segmentCommand.vmaddr(), null));
                    break;
                }
                ...
                if (segmentCommand.vmsize() == 0) {
                    regions.add(new MemRegion(begin, begin, 0, libraryFile, segmentCommand.vmaddr(), null));
                    break;
                }
            ...
            case SEGMENT_64: {
                ...
                if ("__PAGEZERO".equals(segmentCommand64.segname())) {
                    regions.add(new MemRegion(begin, begin + segmentCommand64.vmsize(), 0, libraryFile,
                            segmentCommand64.vmaddr(), null));
                    break;
                }
                ...
                if (segmentCommand64.vmsize() == 0) {
                    regions.add(new MemRegion(begin, begin, 0, libraryFile, segmentCommand64.vmaddr(), null));
                    break;
                }
                ...
                if (log.isDebugEnabled()) {
                    log.debug("mem_map address=0x" + Long.toHexString(alignment.address) + ", size=0x"
                            + Long.toHexString(alignment.size));
                }
        ...
        if (log.isDebugEnabled()) {
            log.debug("load dyId=" + dyId + ", base=0x" + Long.toHexString(loadBase) + ", dyldInfoCommand="
                    + dyldInfoCommand + ", loadNeeded=" + loadNeeded + ", regions=" + regions
                    + ", isPositionIndependent=" + isPositionIndependent);
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("load dyId=" + dyId + ", base=0x" + Long.toHexString(loadBase) + ", neededLibraries="
                    + neededLibraries + ", upwardLibraries=" + upwardLibraries);
        }

        final long loadSize = size;
        MachOModule module = new MachOModule(machO, dyId, loadBase, loadSize,
                new HashMap<String, Module>(neededLibraries), regions, symtabCommand, dysymtabCommand, buffer,
                lazyLoadNeededList, upwardLibraries, exportModules, dylibPath, emulator, dyldInfoCommand, null, null,
                vars, machHeader, isExecutable);
        ...
        while (!done && buffer.hasRemaining()) {
            ...
            case REBASE_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB:
                ...
                if (segmentIndex >= regions.size()) {
                    throw new IllegalStateException(String.format(
                            "REBASE_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB has segment %d which is too large (0..%d)",
                            segmentIndex, regions.size() - 1));
                }
    ...
    private void rebaseAt(Log log, int type, long address, Module module) {
        ...
        /*
         * if (log.isDebugEnabled()) { log.debug("rebaseAt type=" + type +
         * ", address=0x" + Long.toHexString(address - module.base) + ", module=" +
         * module.name + ", old=" + old + ", new=" + newPointer); }
         */
        switch (type) {
    ...
    private void bindLocalRelocations(MachOModule module) {
        ...
        for (int i = 0; i < dysymtabCommand.nLocRel(); i++) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("bindLocalRelocations address=0x" + Integer.toHexString(relocation.address) + ", symbolNum=0x"
                        + Integer.toHexString(relocation.symbolNum) + ", target=0x" + Long.toHexString(target));
            }
        }
    }
    ...
    private boolean bindExternalRelocations(MachOModule module) throws IOException {
        ...
        for (int i = 0; i < dysymtabCommand.nExtRel(); i++) {
            ...
            if (address == 0L) {
                ...
                log.debug("bindExternalRelocations address=0x" + Long.toHexString(relocation.address) + ", symbolNum=0x"
                        + Integer.toHexString(relocation.symbolNum) + ", symbolName=" + symbol.getName());
            }
        }
        return ret;
    }
    ...
    private long resolveSymbol(Module module, Symbol symbol) throws IOException {
        ...
        for (HookListener listener : hookListeners) {
            long hook = listener.hook(emulator.getSvcMemory(), replace == null ? module.name : replace.getModuleName(),
                        ...
                        if ("__dyld".equals(section.sectName())) {
                            ...
                            if (dyldFuncLookup == null) {
                                dyldFuncLookup = emulator.getSvcMemory().registerSvc(new ArmSvc() {
                                    @Override
                                    public long handle(Emulator emulator) {
                                        String name = UnicornPointer.register(emulator, ArmConst.UC_ARM_REG_R0)
                                                .getString(0);
                        ...
                        if ("__dyld".equals(section.sectName())) {
                            ...
                            if (dyldFuncLookup == null) {
                                dyldFuncLookup = emulator.getSvcMemory().registerSvc(new Arm64Svc() {
                                    @Override
                                    public long handle(Emulator emulator) {
                                        String name = UnicornPointer.register(emulator, Arm64Const.UC_ARM64_REG_X0)
                                                .getString(0);
                    ...
                    for (MachO.SegmentCommand.Section section : segmentCommand.sections()) {
                        ...
                        for (int i = 0; i < elementCount; i++, ptrToBind += emulator.getPointerSize()) {
                            ...
                            if (symbolIndex == INDIRECT_SYMBOL_LOCAL) {
                                ...
                                if (log.isDebugEnabled()) {
                                    log.debug("bindIndirectSymbolPointers pointer=" + pointer + ", newPointer="
                                            + newPointer);
                                }
                                pointer.setPointer(0, newPointer);
                                continue;
                            ...
                            if (address == 0L) {
                                if (isWeakRef) {
                                    log.info(
                                            "bindIndirectSymbolPointers symbol=" + symbol + ", isWeakRef=" + isWeakRef);
                                ...
                                pointer.setPointer(0, UnicornPointer.pointer(emulator, address));
                                if (log.isDebugEnabled()) {
                                    log.debug("bindIndirectSymbolPointers symbolIndex=0x"
                                            + Long.toHexString(symbolIndex) + ", symbol=" + symbol + ", ptrToBind=0x"
                                            + Long.toHexString(ptrToBind));
                                }
                            }
                        }
                    }
    ...
    private boolean eachBind(Log log, ByteBuffer buffer, MachOModule module, boolean lazy) throws IOException {
        ...
        while (!done && buffer.hasRemaining()) {
            ...
            case BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB:
                ...
                if (segmentIndex >= regions.size()) {
                    throw new IllegalStateException(String.format(
                            "BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB has segment %d which is too large (0..%d)",
                            segmentIndex, regions.size() - 1));
                }
                MemRegion region = regions.get(segmentIndex);
                address = region.begin + Utils.readULEB128(buffer).longValue();
            ...
            default:
                throw new IllegalStateException(
                        String.format("bad bind opcode 0x%s in bind info", Integer.toHexString(opcode)));
            }
        }
        return ret;
    }

    private boolean doBindAt(Log log, long libraryOrdinal, int type, long address, String symbolName, int symbolFlags,
            long addend, MachOModule module, boolean lazy) throws IOException {
        Symbol symbol = module.findSymbolByName(symbolName, true);
        if (symbol == null) {
            log.warn("doBindAt type=" + type + ", symbolName=" + symbolName + ", address=0x"
                    + Long.toHexString(address - module.base) + ", lazy=" + lazy + ", upwardLibraries="
                    + module.upwardLibraries + ", libraryOrdinal=" + libraryOrdinal);
            return false;
        }
        Pointer pointer = UnicornPointer.pointer(emulator, address);
        if (pointer == null) {
        ...
        /*
         * if (log.isDebugEnabled()) { log.debug("doBindAt 0x=" +
         * Long.toHexString(symbol.getValue()) + ", type=" + type + ", symbolName=" +
         * symbolName + ", symbolFlags=" + symbolFlags + ", addend=" + addend +
         * ", address=0x" + Long.toHexString(address - module.base) + ", lazy=" + lazy +
         * ", symbol=" + symbol + ", libName=" + module.name); }
         */
    ...
    public long mmap2(long start, int length, int prot, int flags, int fd, int offset) {
        ...
        if (((flags & cn.banny.unidbg.ios.MachO.MAP_ANONYMOUS) != 0) || (start == 0 && fd <= 0 && offset == 0)) {
            ...
            log.debug("mmap2 addr=0x" + Long.toHexString(addr) + ", mmapBaseAddress=0x"
                    + Long.toHexString(mmapBaseAddress) + ", start=" + start + ", fd=" + fd + ", offset=" + offset
                    + ", aligned=" + aligned);
        ...
        try {
            ...
            if (start == 0 && fd > 0 && (file = syscallHandler.fdMap.get(fd)) != null) {
                long addr = allocateMapAddress(0, aligned);
                log.debug("mmap2 addr=0x" + Long.toHexString(addr) + ", mmapBaseAddress=0x"
                        + Long.toHexString(mmapBaseAddress));
                return file.mmap2(unicorn, addr, aligned, prot, offset, length, memoryMap);
            }

            if ((flags & MAP_FIXED) != 0) {
                if (log.isDebugEnabled()) {
                    log.debug("mmap2 MAP_FIXED start=0x" + Long.toHexString(start) + ", length=" + length + ", prot="
                            + prot + ", fd=" + fd + ", offset=0x" + Long.toHexString(offset));
                }
        ...
        throw new AbstractMethodError("mmap2 start=0x" + Long.toHexString(start) + ", length=" + length + ", prot=0x"
